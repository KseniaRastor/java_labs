...
public class JCallable {
    ...
    JCallable(double dLowerLimitValue, double dUpperLimitValue, double dStepValue) {
    ...
    public Double call() {
        ...
        for (int i = 0; i < dIterationCountWhole; i++) {
            dSum += dStepValue / 2 * ((1 / dTempA) + (1 / (dTempA + dStepValue)));
            dTempA += dStepValue;
        }
        
        if (dLastStep > 0) {
            dSum += ((1 / dTempA) + (1 / (dTempA + dLastStep))) * dLastStep / 2;
        }
        ...
        synchronized (NewJFrame.class) {
            NewJFrame.addToGlobalResult(dSum);
        }
